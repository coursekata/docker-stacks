...
define build_image
	$(call validate_env,$(1))
	@printf '\n$(info)Building $(OWNER)/$(1)$(3)$(sgr0)\n'
	docker buildx build --tag $(OWNER)/$(1)$(3) \
		--platform $(2) \
	  --build-arg PIXI_ENV="$(1)" \
	  --secret id=github_token,env=GITHUB_TOKEN \
	  .
	@printf "Docker image $(OWNER)/$(1) built successfully for $(1).\n"
endef

# Test an image for a given platform
define test_image
	$(call validate_env,$(1))
	@printf '\n$(info)Testing $(OWNER)/$(1)$(3)$(sgr0)\n'
	@docker run $(DS_RUN_ARGS) --rm \
		...
		"$(OWNER)/$(1)$(3)" $(SHELL) /tmp/test.sh
endef

# Run an image for a given platform
define run_image
	$(call validate_env,$(1))
	docker run -it --rm --platform -p 8888:8888 $(2) $(OWNER)/$(1)$(3)
endef

# Run an image and open a shell for a given platform
define run_shell
	$(call validate_env,$(1))
	docker run -it --rm --platform $(2) $(OWNER)/$(1)$(3) $(SHELL)
endef
